...
@pytest.mark.parametrize(
    "template__name,template__engine", [("Test Name", models.Template.DOCX_TEMPLATE)]
)
...
def test_template_update(db, client, template):
    url = reverse("template-detail", args=[template.pk])
...
@pytest.mark.parametrize(
    "template__name,template__engine,template__template",
    [
        (
            "TestNameTemplate",
        ...
        (
            ...
            # TODO: Add proper mail merge example
            django_file("docx-template.docx"),
        ),
    ],
)
def test_template_merge_docx(db, client, template, snapshot):
    ...
    try:
        ...
        with open(f"/tmp/{template.name}.docx", "wb") as output:
